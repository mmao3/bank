...
public class Withdraw extends JFrame implements ActionListener {
		...
		if (e.getSource() == button) {
			...
			if (amount_.getText().length() == 0) {
				...
				if (type.getSelectedIndex() == 0) {
					try {
						CM.withdraw(accountNumber, Double.parseDouble(amount_.getText().trim()));
					...
					try {
						SM.withdraw(accountNumber, Double.parseDouble(amount_.getText().trim()));
						JOptionPane.showMessageDialog(button, "Your transaction is successful");
					} catch (IllegalArgumentException E) {
						JOptionPane.showMessageDialog(button, "Amount is illegal!");
	...
	public String Balance() {
		...
		return "Checking: " + CM.getBalance(accountNumber) + "  Saving: " + SM.getBalance(accountNumber);
	...
	public Withdraw() {
		...
		System.out.println(CM.getBalance(accountNumber));
		System.out.println(SM.getBalance(accountNumber));
		setSize(300, 300);
		setVisible(true);
		setTitle("Withdraw");
	...
	public Withdraw(int accountNumber) {
		...
		amount_ = new JTextField();
		amount_.setBounds(140, 100, 90, 24);
		contentpane.add(amount_);
		button = new JButton("OK");
		button.setBounds(40, 150, 80, 24);
		contentpane.add(button);
